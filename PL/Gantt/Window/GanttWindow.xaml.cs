...
public partial class GanttWindow : Window
{
    ...
    private void setTasksOnGantt()
    {
        ...
        foreach (var task in Tasks)
        {
            ...
            {
                var dependensTasksSet = task.Dependencies.Select(task => task.Id).ToHashSet();

                var dependensTasks = Tasks.Where(taskItem => dependensTasksSet
                                          .Contains(taskItem.Id))
                                          .OrderBy(task => task.ScheduledDate);

                foreach (var dependsTask in dependensTasks) addGanttRow(dependsTask, rowGroup);
            }
